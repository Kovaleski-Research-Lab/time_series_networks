# Creates an LSTM model using the PyTorch Lightning framework
...
class LSTMLightning(pl.LightningModule):
    def __init__(self, params:dict):
        ...
        try:
            self.lstm = nn.LSTM(input_size=self.params['input_size'],
    ...
    def forward(self, x:torch.Tensor) -> list[torch.Tensor]:
        lstm_out, (lstm_hidden, lstm_cellState) = self.lstm(x)
        prediction = self.fc(lstm_out[:, -1, :])
        return [prediction, lstm_out, lstm_hidden, lstm_cellState]

    def training_step(self, batch:list[torch.Tensor], batch_idx:int) -> torch.Tensor:
        x, y = batch
        prediction, lstm_out, lstm_hidden, lstm_CellState = self.forward(x)
...
if __name__ == "__main__":
    ...
    # Run the model
    prediction, lstm_out, lstm_hidden, lstm_cellState = model(fake_data)
    print(prediction.shape)
    print(lstm_out.shape)
    print(lstm_hidden.shape)
    print(lstm_cellState.shape)
